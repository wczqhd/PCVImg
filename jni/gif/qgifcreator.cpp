...
#define qBound(a,b,c) max(a,min(b,c))
...
void QGifCreator::prepareFrame(IplImage* img, ColorMapObject* map, bool dither)
{
	if(!map && cmaps.size())
	...
	const int bytesPerLine = img->widthStep; //wynik INNY niz img->bytesPerLine()
	Frame frame(npix);
	...
	for(int rw=0;rw<img->height;rw++)
	{
		...
		for(int i=0;i+2<bytesPerLine;i+=step)
		{
			...
			frame[f] = mi; //indeks najbli偶szego koloru
			...
			if(dither)
			{
				...
				//prawy
				if(i+2+step < img->widthStep)
				{
					line[i+step] = 
						qBound(0,(int)(line[i+step]+(float)7/16*qerror[0]),255);//鐩稿綋浜�max(min,min(value,max))
					line[i+1+step] = 
					...
					if(rw+1<img->height)
					{
						...
						uchar* nextLine = (uchar*)(img->imageData + (rw+1)*img->widthStep);
